...
#define _ON_INIT1(x)                                    \
	static void _plugin_init_fn_##x();                        \
	static PluginInitHandler _plugin_init_##x(_plugin_init_fn_##x); \
	static void _plugin_init_fn_##x()
#define _ON_INIT(x) _ON_INIT1(x)
#define ON_INIT _ON_INIT(__COUNTER__)

#define _ON_EVENT1(ev, x, pri)                                            \
	static void _plugin_event_fn_##x(Event::EventData<Event::ev> event);        \
	ON_INIT { Event::RegisterCallback<Event::ev>(&_plugin_event_fn_##x, pri); } \
	static void _plugin_event_fn_##x(Event::EventData<Event::ev> event)
...
class PluginInitHandler {
public:
	PluginInitHandler(std::function<void()> cb);
	std::function<void()> cb;

	static std::vector<PluginInitHandler *> handlers;
	static void RunAll();
};
